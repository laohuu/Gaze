...
#if MESH_DEBUG_LOG
#define GZ_MESH_LOG(...) GZ_CORE_TRACE(__VA_ARGS__)
#else
#define GZ_MESH_LOG(...)
#endif
    ...
    struct LogStream : public Assimp::LogStream
    {
        ...
        virtual void write(const char* message) override { GZ_CORE_ERROR("Assimp error: {0}", message); }
    };
    ...
    Mesh::Mesh(const std::string& filename) : m_FilePath(filename)
    {
        ...
        GZ_CORE_INFO("Loading mesh: {0}", filename.c_str());

        m_Importer = std::make_unique<Assimp::Importer>();

        const aiScene* scene = m_Importer->ReadFile(filename, s_MeshImportFlags);
        if (!scene || !scene->HasMeshes())
            GZ_CORE_ERROR("Failed to load mesh file: {0}", filename);

        m_Scene = scene;

        m_IsAnimated       = scene->mAnimations != nullptr;
        m_MeshShader       = m_IsAnimated ? Renderer::GetShaderLibrary()->Get("PBR_Anim") :
                                            Renderer::GetShaderLibrary()->Get("PBR_Static");
        ...
        for (size_t m = 0; m < scene->mNumMeshes; m++)
        {
            ...
            GZ_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
            GZ_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
            ...
            {
                GZ_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
        ...
        if (m_IsAnimated)
        {
            for (size_t m = 0; m < scene->mNumMeshes; m++)
            {
                ...
                for (size_t i = 0; i < mesh->mNumBones; i++)
                {
                    ...
                    {
                        GZ_MESH_LOG("Found existing bone in map");
        ...
        {
            GZ_MESH_LOG("---- Materials - {0} ----", filename);
            ...
            for (uint32_t i = 0; i < scene->mNumMaterials; i++)
            {
                ...
                GZ_MESH_LOG("  {0} (Index = {1})", aiMaterialName.data, i);
                aiString aiTexPath;
                uint32_t textureCount = aiMaterial->GetTextureCount(aiTextureType_DIFFUSE);
                GZ_MESH_LOG("    TextureCount = {0}", textureCount);
                ...
                GZ_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
                GZ_MESH_LOG("    ROUGHNESS = {0}", roughness);
                GZ_MESH_LOG("    METALNESS = {0}", metalness);
                ...
                if (hasAlbedoMap)
                {
                    ...
                    GZ_MESH_LOG("    Albedo map path = {0}", texturePath);
                    ...
                    {
                        GZ_CORE_ERROR("Could not load texture: {0}", texturePath);
                        fallback = true;
                    }
                }
                ...
                if (hasNormalMap)
                {
                    ...
                    parentPath /= std::string(aiTexPath.data);
                    std::string texturePath = parentPath.string();
                    GZ_MESH_LOG("    Normal map path = {0}", texturePath);
                ...
                if (hasRoughnessMap)
                {
                    ...
                    parentPath /= std::string(aiTexPath.data);
                    std::string texturePath = parentPath.string();
                    GZ_MESH_LOG("    Roughness map path = {0}", texturePath);
                    auto texture = Texture2D::Create(texturePath);
                    if (texture->IsLoaded())
                    {
                        m_Textures.push_back(texture);
                ...
                if (fallback)
                {
                    ...
                    if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
                    {
                        ...
                        if (texture->IsLoaded())
                        {
                            GZ_MESH_LOG("    Metalness map path = {0}", texturePath);
                    ...
                    {
                        GZ_MESH_LOG("    No metalness texture");
                ...
                for (uint32_t p = 0; p < aiMaterial->mNumProperties; p++)
                {
                    ...
                    GZ_MESH_LOG("Material Property:");
                    GZ_MESH_LOG("  Name = {0}", prop->mKey.data);
                    // GZ_MESH_LOG("  Type = {0}", prop->mType);
                    // GZ_MESH_LOG("  Size = {0}", prop->mDataLength);
                    float data = *(float*)prop->mData;
                    GZ_MESH_LOG("  Value = {0}", data);

                    switch (prop->mSemantic)
                    {
                        case aiTextureType_NONE:
                            GZ_MESH_LOG("  Semantic = aiTextureType_NONE");
                            break;
                        case aiTextureType_DIFFUSE:
                            GZ_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
                            break;
                        case aiTextureType_SPECULAR:
                            GZ_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
                            break;
                        case aiTextureType_AMBIENT:
                            GZ_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
                            break;
                        case aiTextureType_EMISSIVE:
                            GZ_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
                            break;
                        case aiTextureType_HEIGHT:
                            GZ_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
                            break;
                        case aiTextureType_NORMALS:
                            GZ_MESH_LOG("  Semantic = aiTextureType_NORMALS");
                            break;
                        case aiTextureType_SHININESS:
                            GZ_MESH_LOG("  Semantic = aiTextureType_SHININESS");
                            break;
                        case aiTextureType_OPACITY:
                            GZ_MESH_LOG("  Semantic = aiTextureType_OPACITY");
                            break;
                        case aiTextureType_DISPLACEMENT:
                            GZ_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
                            break;
                        case aiTextureType_LIGHTMAP:
                            GZ_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
                            break;
                        case aiTextureType_REFLECTION:
                            GZ_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
                            break;
                        case aiTextureType_UNKNOWN:
                            GZ_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
                            break;
                    }
#endif
    ...
    void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
    {
        ...
        // GZ_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
    ...
    {
        GZ_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
    ...
    {
        GZ_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
    ...
    glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
    {
        ...
        GZ_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
        float DeltaTime =
    ...
    glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
    {
        ...
        GZ_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
    ...
    void Mesh::DumpVertexBuffer()
    {
        ...
        GZ_MESH_LOG("------------------------------------------------------");
        GZ_MESH_LOG("Vertex Buffer Dump");
        GZ_MESH_LOG("Mesh: {0}", m_FilePath);
        if (m_IsAnimated)
        {
            for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
            {
                auto& vertex = m_AnimatedVertices[i];
                GZ_MESH_LOG("Vertex: {0}", i);
                GZ_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
                GZ_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
                GZ_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
                GZ_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
                GZ_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
                GZ_MESH_LOG("--");
            }
        }
        else
