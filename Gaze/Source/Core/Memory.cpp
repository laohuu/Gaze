...
namespace Gaze
{
    ...
    static Gaze::AllocationStats s_GlobalStats;
...
#ifndef GZ_DIST
            if (!found)
                GZ_CORE_WARN("Memory", "Memory block {0} not present in alloc map", memory);
#endif
        }
...
_VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size) { return Gaze::Allocator::Allocate(size); }
...
_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size)
_VCRT_ALLOCATOR
void* __CRTDECL operator new(size_t size, const char* desc) { return Gaze::Allocator::Allocate(size, desc); }
...
{
    return Gaze::Allocator::Allocate(size, file, line);
}
...
void __CRTDECL operator delete(void* memory) { return Gaze::Allocator::Free(memory); }

void __CRTDECL operator delete(void* memory, const char* desc) { return Gaze::Allocator::Free(memory); }

void __CRTDECL operator delete(void* memory, const char* file, int line) { return Gaze::Allocator::Free(memory); }
