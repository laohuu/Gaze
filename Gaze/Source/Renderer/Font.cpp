...
//        texture->Unlock();
    ...
    Font::Font(const std::filesystem::path& filepath) : m_Data(new MSDFData())
    {
        ...
        if (!font)
        {
            GZ_CORE_ERROR("Failed to load font: {}", fileString);
            return;
        }
        ...
        GZ_CORE_INFO("Loaded {} glyphs from font (out of {})", glyphsLoaded, charset.size());
...
#define THREAD_COUNT 8
    ...
    Ref<Font> Font::GetDefault()
    {
        ...
        if (!DefaultFont)
            DefaultFont = Ref<Font>::Create("Assets/Fonts/OpenSans/static/OpenSans-Regular.ttf");
